/*
 * Affine 3D Mutli-Resolution Registration. The registration is performed
 * between two volumes FIXED and MOVING and provided a 3D transformation M. 
 * Some mesh MESHi are linked to FIXED, each MESHi is transformed with M^-1
 * they are then write on HD.
 *
 ...
 *	* The transform inverse path
 *	* Init translation in mm (optional, default 0 0 0 )
 *	* Ratio of pixel to take into account - between 1 and 100 (optional, 
 ...
 *
 *
 * Mostly inspirited by ImageRegistration8.cxx and
...
//#include "lib/simpleCommandIterationUpdate.h"

// observers
#include "../lib/simpleObserver.h"
// affine transform object
#include "../lib/affineMultiRegistration.h"
...
int main( int argc, char *argv[] )
{
  ...
  float translationScale = 1.0 / 1; //1e5
	int ratio = 100;
	int maxIter = 200;
	int nbrOfLevel = 3; // min size is along z and is 32 for GP
	if( argc < 6 )
    {
	...
	const int argvFixedImageFile=1, argvMovingImageFile=2,
			...
			argvTranslationScale=11, argcIter=12, argcLevel=13;
	...
	initTranslation[1] = 0;
	initTranslation[2] = 0;
	if( argc >= argcLevel+1 ) nbrOfLevel = atoi( argv[argcLevel] );
	if( argc >= argcIter+1 ) maxIter = atoi( argv[argcIter] );
	...
	std::cout << "Inputs: Tx,Ty,Tz=" << initTranslation
		...
		<< " momentScale=" << momentScale << " #iter=" << maxIter 
		<< " #levels=" << nbrOfLevel
		<< std::endl;
  ...
  //typedef itk::Image< MaskPixelType, Dimension >   MaskImageType;
	...
	doRegistration->SetMaxIter( maxIter );
	doRegistration->SetMovingImage( movingImage );
	doRegistration->SetFixedImage( fixedImage );
