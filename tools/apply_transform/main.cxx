 ...
 *       Filename:  main.cxx
 *
 *    Description: deform an input image with affine input
 *								transforms.
...
int main( int argc, char *argv[] )
{
  ...
  if( argc < 5 )
	...
	const unsigned int argvInputImage = 1, argvInputTransform = 2,
							 ...
							 argcIsTransformAtCenter = 5;

	bool isTransformAtImageCenter = false;
	if( (argc > argcIsTransformAtCenter) && (atoi(argv[argcIsTransformAtCenter]) == 1) ){
		isTransformAtImageCenter = true;
		std::string m = "Transformation will be done in the center of the volume.";
		blueMessage(m);
	}
  ...
  itk::TransformFileReader::Pointer transformReader =
	...
	for( unsigned int i=0; i<Dimension; i++)
		center[i] = deformed->GetOrigin()[i] + deformed->GetLargestPossibleRegion().GetSize()[i]/deformed->GetSpacing()[i]/2;
